...
release that focused on getting Gleam ready for a v1.0 release.
...
If a module defines this type when you read the code
`import the/module.{Person}` you cannot tell if the type is being imported, the
value, or both. This ambiguity is resolved by the compiler by checking to see
how the imported item is used.
...
Run `gleam fix` in the root of any Gleam project and it will automatically
...
Emacs, and any other edit that supports the protocol.
