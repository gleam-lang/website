...
[language server](https://langserver.org/), and, a headless IDE engine that adds
...
Autocompletion is provided for types, as well as module level functions and
constants either imported or defined within the current module.
...
Gleam code compiles really fast! Both because of the compiler performance, but
also because it supports incremental compilation to avoid recompiling code that
hasn't changed.
