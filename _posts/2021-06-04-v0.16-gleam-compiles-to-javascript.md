...
Rather than attempting to replicate a subset of Erlang's actor based concurrency
model Gleam uses JavaScript's standard promise based concurrency model. While
...
can be called like normal from languages like JavaScript and TypeScript.
...
`Promise.prototype.then`, or with the `await` keyword, while in Erlang or Go this
is handled automatically by the compiler.

The pervasive `Promise` type and the split between synchronous and asynchronous
functions in JavaScript can be more difficult to learn and to some less
enjoyable to use than languages such as Erlang and Go that make no distinction.
While there are no firm plans at present I would like to explore using the Gleam
compiler's static analysis to automatically insert `await` statements into the
